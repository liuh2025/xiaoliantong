...
- **准出条件**:
  - [x] DESN 设计文档 已批准
  - [x] CHK 一致性检查 已批准
  - [x] QA-test-plan 测试计划 已批准
  - [x] 开发 worktree 创建完毕（8个模块）
...
- **准出条件**:
  - [x] 所有 task 状态 = 已完成
  - [x] 单元测试 100% 通过 (753 passed)
  - [x] TL 全量代码审查通过
- **Worktree 列表**:
  - `feature/auth_app` - 认证模块
  - `feature/ent` - 企业名录
  - `feature/opp` - 商机广场
  - `feature/feed` - 校友圈
  - `feature/ent-admin` - 企业端管理
  - `feature/plat-admin` - 平台端管理
  - `feature/msg` - 消息通知
  - `feature/search` - 搜索服务
...
### M3 开发阶段准出（m3-exit）
- [x] 所有 task 状态 = 已完成（由 m3-subagent-development 管理）
- [x] 静态代码扫描无阻断问题（flake8 / bandit）
- [x] 单元测试 100% 通过
- [x] TL 全量代码审查通过（集成在 m3-subagent-development 中）

### M4 测试阶段准出（m4-exit）
- [x] TEST-REPORT 无待执行项
- [x] TEST-REPORT 无待修复项
- [x] L1 API 测试 100% 通过
- [x] L2 E2E 截图完整（tests/e2e/screenshots/）
- [x] L2 E2E 测试 100% 通过
- [x] QA 已签署审查意见
