...
> 以下任务因依赖其他模块而暂缓，待依赖模块开发完成后由 TL 派发补全。

| 任务ID | 依赖模块 | 所属模块 | 描述 | 触发条件 |
|--------|----------|----------|------|----------|
| ENT-003-DEP | opp | ent | 企业详情接口的 `opportunities` 字段：查询该企业最新3条商机（status=ACTIVE），非VERIFIED返回空列表。当前返回 `[]` 占位 | opp 模块 OPP-001 完成后 |
