...
## API 字段定义规范（DESN 编写强制要求）

**目的**：API 接口字段定义是集成测试的唯一依据，必须与数据库设计保持一致。
...
| **路径参数** | 字段名、类型、必填、说明 |
| **查询参数** | 字段名、类型、必填、说明 |
| **请求体字段** | 字段名、类型、必填、说明（对应数据库字段） |
| **响应体字段** | 字段名、类型、说明 |

### 一致性检查要求

1. **API 字段与数据库字段一一对应**
   - API 请求/响应的字段必须在对应的数据库表中有明确的字段定义
   - 字段类型必须匹配（如 VARCHAR → string, INT → int, DATETIME → datetime）
   - 必填状态必须一致

2. **示例对应关系**
   ```
   数据库: ent_enterprise.credit_code VARCHAR(18) Y
   API请求: credit_code string Y 统一社会信用代码，18位
   API响应: credit_code string 统一社会信用代码
   ```

3. **禁止出现的情况**
   - API 文档中有字段但数据库中无对应字段
   - 数据库中必填字段但 API 文档中标记为可选
   - API 与数据库字段类型不匹配

### API 测试用例编写依据

集成测试（API 测试）必须基于 DESN 中的 API 字段定义：
- 每个 API 的每个字段都需要有正例和反例测试
- 测试数据必须符合字段约束（如长度、格式、必填）
- 测试断言必须验证响应字段与 DESN 定义一致
